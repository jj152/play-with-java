...
public class StreamExample03 {
	...
	public static void main(String[] args) {
		Person[] peopleArr = { new Person("Brandon", 23), new Person("Hank", 43), new Person("Jenna", 33),
				new Person("Veronica", 56), new Person("Jack", 27), };
		...
		Car[] carsArr = { new Car("Chevy", "red", 45000f), new Car("Ford", "blue", 23000f),
				new Car("Toyota", "grey", 14000f), new Car("Lamborghini", "blue", 150000f),
				new Car("Renault", "blue", 150000f), };
		List<Car> cars = new ArrayList<>(Arrays.asList(carsArr));

		// Answer 2 goes here
		List<Car> blueCarsList = cars.stream().filter((car) -> car.color == "blue").collect(Collectors.toList());
		blueCarsList.stream().forEach((car) -> System.out.println(car.make));
		...
		// Answer 3 goes here
		Float salarySum = employees.stream().map((e)-> e.salary).reduce(0f, (acc, x) -> {
			Float result = acc + x;
